...
export default function LaunchServiceModal({ patient, onClose, onServiceLaunched }: LaunchServiceModalProps) {
        ...
        if (patient) {
            ...
            api.get('/users').then(res => {
                ...
                if (professionalList.length > 0) {
                    setSelectedProfessional(professionalList[0].user_id);
                }
            }).catch(() => {
    ...
    const handleConfirm = async () => {
        ...
        try {
            const response = await api.post('/appointments/on-demand', {
        ...
        <div className="fixed inset-0 bg-black bg-opacity-60 flex items-center justify-center z-50 p-4">
            <div className="bg-white rounded-lg shadow-xl p-6 w-full max-w-md animate-fade-in-up">
                ...
                <div className="space-y-4">
                    ...
                    <div>
                        <label htmlFor="professional" className="block text-sm font-medium text-gray-700">Direcionar para o Profissional:</label>
                        <select id="professional" value={selectedProfessional} onChange={(e) => setSelectedProfessional(e.target.value)}
